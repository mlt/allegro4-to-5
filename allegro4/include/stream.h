...
AL_INLINE(void *, get_audio_stream_buffer, (AUDIOSTREAM *stream), {
    return stream->locked = al_get_audio_stream_fragment(stream->samp->stream);
});
AL_INLINE(void, free_audio_stream_buffer, (AUDIOSTREAM *stream), {
    al_set_audio_stream_fragment(stream->samp->stream, stream->locked);
});
